...
<?php
    if (isset($_GET['id'])) {//ID
        $id = $_GET['id'];
        $projAExcluir = R::load('proj', $id);//CARREGA O PROJ DE TAL ID
        if(isset($_POST['decisao'])){
            if($_POST['decisao'] == 'sim'){
                R::trash($projAExcluir);
                echo "<h3 style='color:green;text-align:center;'>Projeto excluído com sucesso!</h3>";
                ...
                echo "<h3 style='color:darkred;text-align:center;'>Projeto não excluído!</h3>";
                echo "<table style='margin: auto;'><td><ul class='fa-ul'><li><span class='fa-li'><a href='../listagem-proj.php'><i class='fa-solid fa-rotate-left'></i></span>Voltar</a></li></ul></td></table>";
            }
        } else{
?>
<h3 style='text-align:center;'>Excluir '<?=$projAExcluir->nome?>' <span style='color:black;-webkit-text-stroke: 1px darkred;'>permanentemente</span>?</h3>
